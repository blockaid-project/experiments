...
sleep 60
numactl -N 1 -m 1 python3 spree/measure_plt.py "modified" --warmup-rounds "$rounds" --measure-rounds "$rounds" >> "$output_dir/modified.csv"

~/start_spree.sh production &
sleep 60
numactl -N 1 -m 1 python3 spree/measure_plt.py "original" --warmup-rounds "$rounds" --measure-rounds "$rounds" >> "$output_dir/original.csv"

## Cache disabled.
#cp common/no-cache.rbenv-vars ~/spree/sandbox/.rbenv-vars
#~/start_spree.sh production_mod_checked &
#sleep 60
#numactl -N 1 -m 1 python3 spree/measure_plt.py "no-cache" --warmup-rounds "$rounds" --measure-rounds "$rounds" >> "$output_dir/no-cache.csv"
